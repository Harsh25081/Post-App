...
const ShowPost = () => {
    ...
    let [repid, setRepid] = useState("")
    let [editcmnt,setEditcomnt] = useState(false)
    ...
    const edtreply = (comId, replyid, rep) => {
        let _id = comId;
        let replyId = replyid
        axios.put("https://post-app-lake.vercel.app/updatecomment", { _id, replyId, rep })
            .then((res) => { console.log(res.data) })
            .catch((err) => { console.log(err.message) })
        setComm(false);
        getComment();
                                ...
                                {PostId === selId && <div><br />
                                    <div style={{ display: "flex" }}>
                                        ...
                                        <div style={{ border: "1.5px solid black", width: "90%", marginTop: "5px", borderRadius: "10px" }}>
                                            ...
                                            {(editcmnt === false || comId!==repid) && <p>{Comment}</p>}
                                            {(editcmnt === true && comId===repid) && <textarea placeholder={Comment} onChange={(e) => { console.log(e.target.value); setPost(e.target.value); }} style={{ height: "50%", width: "99%", resize: "none", border: "transparent", marginLeft: "2px" }}>{Comment}</textarea>}
                                        </div>
                                        {/* <button>Edit</button>Edit Button */}
                                        {editcmnt === false && <button onClick={() => { setEditcomnt(true);setRepid(comId) }}>Edit</button>}
                                        {(editcmnt === true && comId===repid) && <button onClick={() => editcomment(_id, post)}>Save</button>}
                                    ...
                                    {rep === true && <div style={{ display: "flex" }}>
                                        ...
                                        {rep === true && <button onClick={() => { editreply(comId, replyy, Namee);setRep(false) }}>send</button>}
                                    </div>}
                                        ...
                                        return <div key={index} style={{ marginLeft: "90px" }}>
                                            {repl.status === false && <div style={{ display: "flex" }}>
                                                ...
                                                <div style={{ border: "1.5px solid black", width: "90%", marginTop: "5px", borderRadius: "10px" }}>
                                                    ...
                                                    {(comm === false || replyId !== repid) && <p>{reply}</p>}
                                                    {(comm === true && replyId === repid) && <textarea defaultValue={Reply.find((x) => x._id === repl._id).reply} onChange={(e) => { console.log(e.target.value); setPost(e.target.value); }} style={{ height: "50%", width: "99%", resize: "none", border: "transparent", marginLeft: "2px" }}></textarea>}
                                                </div>
                                                {/* <button>Edit</button>Edit Button */}
                                                {comm === false && <button onClick={() => { setComm(true); setRepid(replyId) }}>Edit</button>}
                                                {(comm === true && replyId === repid) && <button onClick={() => { edtreply(comId, replyId, post)}}>Save</button>}
